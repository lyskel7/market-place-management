...
interface LayoutProps {
  ...
  // NO declares params aquí si no lo usas, pero React.memo lo manejará
}

const LayoutInner = (props: LayoutProps) => {
  ...
  const userInfoFromStore = useAuthStore((state) => {
    console.log(
  ...
  const session = useMemo(() => {
    ...
    return {
      user: {
        name: userInfoFromStore || '...',
        email: userEmail || '...',
        // image: avatarUrl || '',
      },
    };
  }, [userEmail, userInfoFromStore]);

  useEffect(() => {
    console.log(
      'Layout.tsx EFFECT: NAVIGATION reference changed or component remounted.',
    );
  }, []); // Solo se ejecuta si la referencia de NAVIGATION cambia
